[index.html..html](https://github.com/user-attachments/files/28400796/index.html.html)
<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<div class="field"><label>Data de entrada do pedido</label><input id="entrada" type="date"></div>
<div class="field"><label>Entrega</label><input id="entrega" type="date"></div>
<div class="field"><label>Previsão de início</label><input id="previsaoInicio" type="date"></div>
<div class="field"><label>Previsão de conclusão</label><input id="previsaoConclusao" type="date"></div>
<div class="field"><label>Etapa</label><select id="etapa"><option>A produzir</option><option>Em produção</option><option>Galvanização</option><option>Pintura</option><option>Pátio</option><option>Concluído</option><option>Entregue</option></select></div>
...
<div id="topAlert"></div>
<div class="table-wrap"><table><thead><tr><th>Cliente</th><th>Produto</th><th>Resp.</th><th>Prioridade</th><th>Pedido</th><th>Entrada pedido</th><th>Prev. início</th><th>Prev. conclusão</th><th>Entrega</th><th>Etapa</th><th>Atraso</th><th>Ações</th></tr></thead><tbody id="tableBody"></tbody></table></div>
</div>
...
const getDelay=x=>{const ref=x.entrega; if(!ref||x.etapa==='Concluído'||x.etapa==='Entregue') return 0; return Math.max(0,days(today().toISOString().slice(0,10),ref));};
const save=()=>localStorage.setItem('pcpflowv5',JSON.stringify(data));
const stageOrder=['A produzir','Em produção','Galvanização','Pintura','Pátio','Concluído','Entregue'];
const normalizeStage=s=>['A produzir','Em produção','Galvanização','Pintura','Pátio','Concluído','Entregue'].includes(s)?s:'A produzir';
...
function add(){const x={cliente:cliente.value.trim(),produto:produto.value.trim(),responsavel:responsavel.value.trim(),prioridade:prioridade.value,pedido:pedido.value.trim(),entrada:entrada.value,previsaoInicio:previsaoInicio.value,previsaoConclusao:previsaoConclusao.value,entrega:entrega.value,etapa:etapa.value||'A produzir'}; if(!x.cliente||!x.produto){alert('Preencha cliente e produto.'); return} data.unshift(x); ['cliente','produto','responsavel','pedido','entrada','previsaoInicio','previsaoConclusao','entrega'].forEach(id=>document.getElementById(id).value=''); prioridade.value='Baixa'; render();}
